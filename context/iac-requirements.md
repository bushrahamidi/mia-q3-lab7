...
The todo-service is a simple full-stack task management app used as the baseline service for this golden-path lab: a React frontend that calls a Node.js/Express backend exposing CRUD endpoints for `/api/todos` and a `/health` check. The service itself remains intentionally simple (in-memory todo storage) so the focus is on platform delivery.

In this lab, it will be containerized and hosted on AWS using ECS Fargate behind an Application Load Balancer. Terraform configures networking, ECR repositories for backend and frontend images, ECS tasks/services, security groups, and CloudWatch logging/Container Insights, with CI/CD workflows handling image build/push and infrastructure deployment.
